"""
Train PPO Agent for Portfolio Management using Stable-Baselines3.
...
PPO (Proximal Policy Optimization) differs from DDPG/TD3:
1. On-policy algorithm (vs off-policy DDPG/TD3)
2. Policy gradient method (vs actor-critic with Q-learning)
3. Uses clipped surrogate objective for stable updates
4. No replay buffer (learns from current trajectories)

Configuration matches DDPG/TD3 and tabular agents for fair comparison.
"""
...
# ============================================
# Configuration - SAME AS DDPG/TD3/TABULAR FOR FAIR COMPARISON
# ============================================
CONFIG = {
    # Environment (identical to DDPG, TD3, and tabular Q-learning)
    'trading_cost': 0.00025,  # 0.025% = 2.5 bps 
    'max_steps': 256,
    'window_length': 50,
    'seed': SEED,

    # Walk-forward split (identical to DDPG, TD3, and tabular)
    'val_split': 0.2,  # 20% of train data for validation

    # Training
    # PPO is on-policy, so it needs more environment interactions
    # 1M timesteps for fair comparison with DDPG/TD3
    'total_timesteps': 1000000,
    'print_every': 50000,
    'val_every': 200000,

    # PPO Hyperparameters
    'learning_rate': 3e-4,  # PPO default, slightly higher than DDPG
    'n_steps': 2048,  # Steps per update (PPO collects this many before updating)
    'batch_size': 64,  # Minibatch size for PPO updates
    'n_epochs': 10,  # Number of epochs per update
    'gamma': 0.99,  # Discount factor (same as DDPG/TD3/tabular)
    ...
    # Network architecture (same as DDPG/TD3)
    'net_arch': dict(pi=[256, 256], vf=[256, 256]),

    # Reward shaping (identical to DDPG, TD3, and tabular)
    'reward_scale': 100,
    'cost_penalty': 0.1,
...
def main():
    print("=" * 70)
    print("PPO Training for Portfolio Management (Stable-Baselines3)")
    print("=" * 70)
    print(f"Random Seed: {SEED}")
    print("PPO: On-policy, policy gradient with clipped surrogate objective")
    ...
    fig.suptitle('PPO Training (Stable-Baselines3)', fontsize=14)
    ...
    print(f"\nPlot saved to: ppo_training.png")
    plt.show()

    # Save model
    model.save('ppo_agent')
    print(f"Model saved to: ppo_agent.zip")
