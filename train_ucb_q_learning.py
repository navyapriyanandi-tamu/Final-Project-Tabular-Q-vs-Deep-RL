"""
Train UCB-Epsilon Q-Learning Agent for Portfolio Management.

UCB-epsilon combines:
1. Epsilon-greedy exploration (random with probability epsilon)
2. UCB (Upper Confidence Bound) for action selection when not exploring randomly

This balances exploration-exploitation by favoring uncertain actions.

Configuration matches standard Q-Learning for fair comparison.
"""
...
from rl_portfolio_management.agents import UCBQLearningAgent
...
# ============================================
# Configuration - SAME AS Q-LEARNING FOR FAIR COMPARISON
# ============================================
CONFIG = {
    # Environment
    ...
    # Agent (same hyperparameters as standard Q-learning)
    'learning_rate': 0.1,
    'lr_min': 0.01,
    'lr_decay': 0.9999,
    ...
    # UCB-specific parameter
    'ucb_c': 2.0,  # UCB exploration constant

    # Reward shaping ( log-return - λ*cost - drawdown_penalty)
    'reward_scale': 100,
...
def main():
    print("=" * 70)
    print("UCB-Epsilon Q-Learning Training for Portfolio Management")
    print("=" * 70)
    print(f"Random Seed: {SEED}")
    print(f"UCB Constant (c): {CONFIG['ucb_c']}")
    ...
    print(f"  UCB constant (c): {agent.ucb_c}")
    ...
    history = {
        ...
        'epsilon_values': [],
        'lr_values': [],
        'ucb_selections': []  # Track UCB vs random selection
    }
    ...
    for episode in range(1, CONFIG['n_episodes'] + 1):
        ...
        for step in range(CONFIG['max_steps']):
            ...
            was_random = np.random.random() < agent.epsilon
        ...
        history['ucb_selections'].append(ucb_count / max(steps_taken, 1))
        ...
        if episode % CONFIG['print_every'] == 0:
            ...
            recent_ucb = np.mean(history['ucb_selections'][-500:]) * 100
            print(f"Episode {episode:>6} | PV: {recent_pv:.4f} | Reward: {recent_reward:>7.2f} | ε: {agent.epsilon:.4f} | UCB%: {recent_ucb:.1f}%")
    ...
    print(f"Total state visits: {stats['total_visits']}")
    print(f"Avg visits per state: {stats['avg_visits_per_state']:.1f}")
    ...
    fig.suptitle(f'UCB-Epsilon Q-Learning Training (Walk-Forward Split)', fontsize=14)
    ...
    # Validation progress (instead of UCB selections for consistency with Q-Learning plot)
    ax4 = axes[1, 0]
    ...
    plt.tight_layout()
    plt.savefig('ucb_q_learning_training.png', dpi=150)
    print(f"\nPlot saved to: ucb_q_learning_training.png")
    plt.show()

    # Save agent
    agent.save('ucb_q_learning_agent.pkl')
    print(f"Agent saved to: ucb_q_learning_agent.pkl")
