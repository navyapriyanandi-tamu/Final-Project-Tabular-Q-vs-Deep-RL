"""
Train DDPG Agent for Portfolio Management using Stable-Baselines3.

- "DDPG (comparison): PyTorch or Stable-Baselines3 DDPG with identical
   reward/costs/splits for an apples-to-apples stability comparison."
- "Continuous version (for DDPG/TD3/PPO): target weight vector in a Box space,
   projected to the simplex."

Configuration matches tabular Q-Learning for fair comparison.
"""
...
def set_seed(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
    # PyTorch seed is set by SB3 when we pass seed parameter

set_seed(SEED)


# ============================================
# Configuration - SAME AS TABULAR Q-LEARNING FOR FAIR COMPARISON
# ============================================
CONFIG = {
    # Environment (identical to tabular)
    'trading_cost': 0.00025,  # 0.025%
    'max_steps': 256,
    'window_length': 50,
    'seed': SEED,

    # Walk-forward split (identical to tabular)
    'val_split': 0.2,  # 20% of train data for validation

    # Training
    'total_timesteps': 1000000,  # Total training timesteps
    'print_every': 50000,
    'val_every': 200000,  # Evaluate on validation set every N timesteps
    ...
    'gamma': 0.99,  # Discount factor (same as tabular)
    'train_freq': 1,  # Update policy every step
    'gradient_steps': 1,
    'action_noise_std': 0.1,  # Exploration noise

    # Network architecture
    'net_arch': [256, 256],  # Hidden layers for actor and critic

    # Reward shaping (identical to tabular)
    'reward_scale': 100,
    'cost_penalty': 0.1,
...
def main():
    print("=" * 70)
    print("DDPG Training for Portfolio Management (Stable-Baselines3)")
    print("=" * 70)
    print(f"Random Seed: {SEED}")
    print("Using identical reward/costs/splits as tabular agents")
    ...
    # Create DDPG Agent
    # ============================================
    # Add exploration noise (Ornstein-Uhlenbeck for continuous control)
    ...
    print(f"  Action noise: OU with sigma={CONFIG['action_noise_std']}")
    ...
    fig.suptitle('DDPG Training (Stable-Baselines3)', fontsize=14)
    ...
    colors_box = ['lightblue', 'lightgreen', 'lightorange']
    ...
    print(f"\nPlot saved to: ddpg_training.png")
    plt.show()

    # Save model
    model.save('ddpg_agent')
    print(f"Model saved to: ddpg_agent.zip")
