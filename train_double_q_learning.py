"""
Train Double Q-Learning Agent for Portfolio Management.
...
Uses the same configuration as standard Q-learning for fair comparison.
"""
import numpy as np
import random
...
from rl_portfolio_management.agents import DoubleQLearningAgent
...
# ============================================
# Configuration (same as standard Q-learning)
# ============================================
CONFIG = {
    # Environment
...
def main():
    print("=" * 70)
    print("DOUBLE Q-LEARNING Training for Portfolio Management")
    print("=" * 70)
    ...
    for episode in range(1, CONFIG['n_episodes'] + 1):
        ...
        if episode % CONFIG['print_every'] == 0:
            ...
            print(f"Episode {episode:>6} | PV: {recent_pv:.4f} | Reward: {recent_reward:>7.2f} | "
                  f"ε: {agent.epsilon:.4f} | Q1/Q2: {stats['q1_updates']}/{stats['q2_updates']}")
    ...
    print(f"Q1 updates: {stats['q1_updates']}, Q2 updates: {stats['q2_updates']}")
    print(f"Mean overestimation metric: {stats['mean_overestimation']:.4f}")
    ...
    fig.suptitle(f'Double Q-Learning Training (Walk-Forward Split)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('double_q_learning_training.png', dpi=150)
    print(f"\nPlot saved to: double_q_learning_training.png")
    plt.show()

    # Save agent
    agent.save('double_q_learning_agent.pkl')
    print(f"Agent saved to: double_q_learning_agent.pkl")
